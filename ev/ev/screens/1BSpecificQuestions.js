...
const FirstBaseSpecificQuestions = () => {
  ...
  const [defensiveRating, setDefensiveRating] = useState('');
  ...
  const [firstBaseAspects] = useState([
    { key: 'Throwing Accuracy', label: 'Throwing Accuracy' },
    { key: 'Scooping Ability', label: 'Scooping Ability' },
  ...
  // Generate velocity options from 50 to 100 mph
  const velocityOptions = Array.from({ length: 51 }, (_, i) => (i + 50).toString());
  ...
  const handleContinue = () => {
    if (currentQuestion === 1) {
      ...
      setRankedAspects(firstBaseAspects);
  ...
  const renderQuestion = () => {
    switch (currentQuestion) {
      case 1:
        return (
          <>
            <Text style={styles.title}>What Is Your Top Throwing Velocity From First Base?</Text>
      ...
      case 2:
        return (
          <>
            <Text style={styles.title}>How Would You Rate Your Defensive Ability At First Base?</Text>
            <View style={styles.ratingContainer}>
              {[1, 2, 3, 4, 5].map((rating) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.ratingButton,
                    defensiveRating === rating.toString() && styles.selectedRating
                  ]}
                  onPress={() => setDefensiveRating(rating.toString())}
                >
                  <Text
                    style={[
                      styles.ratingNumber,
                      defensiveRating === rating.toString() && styles.selectedRatingText
                    ]}
                  >
                    {rating}
                  </Text>
      ...
      case 3:
        return (
          <>
            <Text style={styles.title}>Rank These Aspects Of Your First Base Play From Weakest To Strongest</Text>
