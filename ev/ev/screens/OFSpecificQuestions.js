...
const OFSpecificQuestions = () => {
  const navigation = useNavigation();
  const [currentQuestion, setCurrentQuestion] = useState(1);
  const [velocity, setVelocity] = useState('');
  const [trackingRating, setTrackingRating] = useState('');
  const [outfieldSkills] = useState([
    ...
    { key: 'Fly Ball Tracking', label: 'Fly Ball Tracking' },
    { key: 'Ground Ball Pickups', label: 'Ground Ball Pickups' }
  ]);
  ...
  // Generate velocity options from 70 to 105 mph
  const velocityOptions = Array.from({ length: 36 }, (_, i) => (i + 70).toString());
  ...
  const handleContinue = () => {
    if (currentQuestion === 1) {
      ...
      setRankedSkills(outfieldSkills);
  ...
  const renderQuestion = () => {
    switch (currentQuestion) {
      case 1:
        return (
          <>
            <Text style={styles.title}>What Is Your Top Throwing Velocity From The Outfield?</Text>
      ...
      case 2:
        return (
          <>
            <Text style={styles.title}>How Would You Rate Your Tracking Ability On Fly Balls?</Text>
            <View style={styles.ratingContainer}>
              {[1, 2, 3, 4, 5].map((rating) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.ratingButton,
                    trackingRating === rating.toString() && styles.selectedRating
                  ]}
                  onPress={() => setTrackingRating(rating.toString())}
                >
                  <Text
                    style={[
                      styles.ratingNumber,
                      trackingRating === rating.toString() && styles.selectedRatingText
                    ]}
                  >
                    {rating}
                  </Text>
      ...
      case 3:
        return (
          <>
            <Text style={styles.title}>Rank These Outfield Skills From Weakest To Strongest</Text>
...
export default OFSpecificQuestions; 